...
class GenerateTFRecord(object):
    ...
    def convert_numpy_folder(self, catalogue, ont_idx, tfrecord_fn):
        ...
        for chain in self.labels:
            ...
            d_feature['mf_labels'] = dtype_feature_y(y[ont_idx[0]])
            d_feature['bp_labels'] = dtype_feature_y(y[ont_idx[1]])
            d_feature['cc_labels'] = dtype_feature_y(y[ont_idx[2]])
...
if __name__ == "__main__":
    ...
    for seqid in [30]:
        path = '/mnt/home/vgligorijevic/Projects/NewMethods/Contact_maps/go_annot/merged_ont/pdb_GO_train_test_split_bc_' + str(seqid) + '.pckl'
        ...
        mf_idx = np.where(ontology == 'molecular_function')[0]
        bp_idx = np.where(ontology == 'biological_process')[0]
        cc_idx = np.where(ontology == 'cellular_component')[0]
        # for split in ['train', 'valid']:
        for split in ['valid']:
            chains = annot[split + '_pdb_chains']
            Y = annot['Y_' + split]
            # for num, evid in enumerate(['EXP', 'EXP-IEA']):
            for num, evid in enumerate(['EXP']):
                print ('pdb_chains_seqid_' + str(seqid) + '_' + split + '_' + evid + '.tfrecords')
                tmp_Y = Y.copy()
                ...
                tmp_Y = tmp_Y[idx]
                tmp_chains = tmp_chains[idx]

                labels = {}
                for i, chain in enumerate(tmp_chains):
                    labels[chain] = tmp_Y[i]
                tfr = GenerateTFRecord(labels)
                tfr.convert_numpy_folder(cmap2path, [mf_idx, bp_idx, cc_idx],
                                         out_path + 'pdb_chains_seqid_' + str(seqid) + '_' + split + '_' + evid + '.tfrecords_')
